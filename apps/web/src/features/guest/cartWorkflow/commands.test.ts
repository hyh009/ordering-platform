...
describe('guest cart workflow order history', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('records a submitted order with the current scoped session token', async () => {
    ...
    vi.mocked(guestCartService.submitCart).mockResolvedValue(order('order-a'));
  ...
  it('records join-to-order history with the token from the join response', async () => {
    const runtime = createGuestRuntime();
    await runtime.commands.tenant.activateStore('store-a');
    vi.mocked(guestCartService.joinCart).mockResolvedValue({
  ...
  it('does not record a stale submit after the scoped session changes', async () => {
    ...
    vi.mocked(guestCartService.submitCart).mockImplementation(
      () =>
        new Promise((resolve) => {
