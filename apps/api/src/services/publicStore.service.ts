...
export class PublicStoreService {
  public async getActivePublicStore(storeId: string): Promise<StoreEntity> {
  ...
  public async getPublicStore(storeId: string): Promise<PublicStoreDto> {
    const store = await this.getActivePublicStore(storeId);
    return toPublicStoreDto(store);
  }

  public async getPublicMenu(storeId: string): Promise<PublicMenuDto> {
    await this.getActivePublicStore(storeId);
    ...
    // already displayOrder-sorted by the repository. Empty categories are
    // dropped; products with no known category go into a trailing null group.
    ...
    for (const category of categories) {
      ...
      groups.push({
        ...
        products: sortProductsByCategoryOrder(
          members,
    ...
    return {
      groups,
      modifiers: filterReferenced(modifiers, referencedModifierIds).map(
        toPublicModifierDto,
      ),
      tags: filterReferenced(tags, referencedTagIds).map(toPublicTagDto),
      allergens: filterReferenced(allergens, referencedAllergenIds).map(
        toPublicAllergenDto,
      ),
      dietaryMarkers: filterReferenced(
        dietaryMarkers,
        referencedDietaryMarkerIds,
      ).map(toPublicDietaryMarkerDto),
    };
  }
}

export function createPublicStoreService() {
  return new PublicStoreService();
}

export const publicStoreService = createPublicStoreService();
