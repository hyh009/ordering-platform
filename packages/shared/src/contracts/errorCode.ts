export const ERROR_CODES = {
  BAD_REQUEST: 'BAD_REQUEST',
  UNAUTHORIZED: 'UNAUTHORIZED',
  FORBIDDEN: 'FORBIDDEN',
  NOT_FOUND: 'NOT_FOUND',
  CONFLICT: 'CONFLICT',
  INTERNAL_SERVER_ERROR: 'INTERNAL_SERVER_ERROR',
  VALIDATION_ERROR: 'VALIDATION_ERROR',
  INVALID_ID: 'INVALID_ID',
  INVALID_FIELD_VALUE: 'INVALID_FIELD_VALUE',
  RESOURCE_ALREADY_EXISTS: 'RESOURCE_ALREADY_EXISTS',
  INVALID_CREDENTIALS: 'INVALID_CREDENTIALS',
  INVALID_REFRESH_TOKEN: 'INVALID_REFRESH_TOKEN',
  USER_ALREADY_EXISTS: 'USER_ALREADY_EXISTS',
  USER_DISABLED: 'USER_DISABLED',
  USER_NOT_FOUND: 'USER_NOT_FOUND',
  ORGANIZATION_ALREADY_EXISTS: 'ORGANIZATION_ALREADY_EXISTS',
  ORGANIZATION_NOT_FOUND: 'ORGANIZATION_NOT_FOUND',
  ORGANIZATION_MEMBERSHIP_ALREADY_EXISTS:
    'ORGANIZATION_MEMBERSHIP_ALREADY_EXISTS',
  ORGANIZATION_MEMBERSHIP_NOT_FOUND: 'ORGANIZATION_MEMBERSHIP_NOT_FOUND',
  ALLERGEN_ALREADY_EXISTS: 'ALLERGEN_ALREADY_EXISTS',
  ALLERGEN_NOT_FOUND: 'ALLERGEN_NOT_FOUND',
  DIETARY_MARKER_ALREADY_EXISTS: 'DIETARY_MARKER_ALREADY_EXISTS',
  DIETARY_MARKER_NOT_FOUND: 'DIETARY_MARKER_NOT_FOUND',
  STORE_NOT_FOUND: 'STORE_NOT_FOUND',
  CATEGORY_NOT_FOUND: 'CATEGORY_NOT_FOUND',
  TAG_NOT_FOUND: 'TAG_NOT_FOUND',
  PRODUCT_MODIFIER_NOT_FOUND: 'PRODUCT_MODIFIER_NOT_FOUND',
  PRODUCT_NOT_FOUND: 'PRODUCT_NOT_FOUND',
  PRODUCT_ALREADY_EXISTS: 'PRODUCT_ALREADY_EXISTS',
  INVALID_GUEST_TOKEN: 'INVALID_GUEST_TOKEN',
  INVALID_JOIN_CODE: 'INVALID_JOIN_CODE',
  CART_NOT_FOUND: 'CART_NOT_FOUND',
  CART_NOT_ACTIVE: 'CART_NOT_ACTIVE',
  CART_ITEM_NOT_FOUND: 'CART_ITEM_NOT_FOUND',
  NOT_ITEM_OWNER: 'NOT_ITEM_OWNER',
  ORDER_NOT_FOUND: 'ORDER_NOT_FOUND',
  ORDER_LOCKED: 'ORDER_LOCKED',
  ORDER_TYPE_NOT_ENABLED: 'ORDER_TYPE_NOT_ENABLED',
  STORE_NOT_OPEN: 'STORE_NOT_OPEN',
  PRODUCT_SOLD_OUT: 'PRODUCT_SOLD_OUT',
  MODIFIER_SELECTION_INVALID: 'MODIFIER_SELECTION_INVALID',
  NETWORK_ERROR: 'NETWORK_ERROR',
  INVALID_API_RESPONSE: 'INVALID_API_RESPONSE',
} as const;

export type ErrorCode = (typeof ERROR_CODES)[keyof typeof ERROR_CODES];
